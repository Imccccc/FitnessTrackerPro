	...
	public static ArrayList<WeekPlan> getPlans(){
		...
		//	int planNum = 0;
			...
			while(result.next()){
				...
				ResultSet result2 = stmt.executeQuery("SELECT DISTINCT WEEKDAY FROM plan where.planid="+planid);
				
				SimpleListProperty<DayPlan> dayplanlist = new SimpleListProperty();
				while(result2.next()){
					
					weekday = result2.getInt("weekday");
					
					Statement stmt3 = con.createStatement();
					ResultSet result3 = stmt.executeQuery("SELECT * FROM plan p INNER JOIN activity a ON p.`activityid` = a.`activityid` WHERE p.`planid`="+planid+"AND p.`weekday` = "+weekday);
